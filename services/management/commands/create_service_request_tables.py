...
class Command(BaseCommand):
    help = 'Create tables for ServiceRequest and Notification models'

    def handle(self, *args, **options):
        # SQL to create ServiceRequest table
        service_request_sql = """
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(service_request_sql)
                cursor.execute(notification_sql)
            
            self.stdout.write(
                'Successfully created ServiceRequest and Notification tables'
            )
